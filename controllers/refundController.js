...
export const refundOrder = async (req, res) => {
  ...
  try {
    ...
    // 🟢 --- FIX: Step 8: Restore stock logic ---
    ...
    for (const item of orderItems) {
      // Add the item's main product ID to the set
      ...
      if (bundleContents.length > 0) {
        // 2. IT IS A BUNDLE: Restore stock for each of its contents
        ...
        // 3. NOT A BUNDLE: Restore stock normally to the variant
        await db
          .update(productVariantsTable) // 👈 Update productVariantsTable
          .set({ stock: sql`${productVariantsTable.stock} + ${item.quantity}` })
          .where(eq(productVariantsTable.id, item.variantId)); // 👈 For the item's variantId
      }
    }
    // 🟢 --- END FIX (Step 8) ---


    // 🟢 --- FIX: Step 9: Invalidate caches ---
